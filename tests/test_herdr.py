...
class Mapping(unittest.TestCase):
    ...
    def test_pane_cwd_wins_and_heals_hint(self):
        panes = [
            {"pane_id": "w1:p1", "workspace_id": "w1", "cwd": str(self.one.root / "alpha"), "foreground_cwd": None},
        ...
        with mock.patch.object(herdr, "panes", return_value=panes), mock.patch.object(herdr, "workspaces", return_value=workspaces):
    ...
    def test_hint_used_when_label_matches(self):
        ...
        with mock.patch.object(herdr, "panes", return_value=[]), mock.patch.object(herdr, "workspaces", return_value=workspaces):
    ...
    def test_label_fallback_only_when_unique(self):
        workspaces = [{"workspace_id": "wA", "label": "one"}, {"workspace_id": "wB", "label": "one"}, {"workspace_id": "wC", "label": "two"}]
        with mock.patch.object(herdr, "panes", return_value=[]), mock.patch.object(herdr, "workspaces", return_value=workspaces):
            live = herdr.map_live([self.one, self.two], heal=False)
        self.assertEqual(live, {"two": "wC"})

    def test_current_feature_from_cwd(self):
        ctx = herdr.Context(workspace_id=None, workspace_cwd=str(self.one.root / "alpha"), focused_pane_cwd=None, workspace_label=None)
        with mock.patch.object(herdr, "panes", return_value=[]), mock.patch.object(herdr, "workspaces", return_value=[]):
