# A thread is a flat top-level workspace
...
exactly one checkout to a workspace. A thread spans several repositories and has no
natural anchor, so the plugin creates an ordinary workspace with `workspace create --cwd
<thread root> --label <thread>`.
...
The sidebar shows the thread by name at top level, without a branch or git status badge.
