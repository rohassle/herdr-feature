# Closing a workspace never deletes a thread

Closing a thread's workspace leaves every file on disk. Removal happens only through the
...
Closed threads accumulate until removed, so `open` (to come back) and `remove` (to let
...
workspace of a thread (the thread workspace and the nested per-repository ones) and
