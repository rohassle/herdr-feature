...
The popup is a board of features, each a thread of work across repositories, with a
progress bar per feature. Progress needs a definition of "done" per worktree.
...
a worktree is done when that pull request is in state `MERGED`. A feature is done when
every worktree is.
...
opens features, progress reads `unknown` and refresh aborts with `gh`'s own message.

Lookups only happen on an explicit refresh (`ctrl-r` on the board, `herdr-feature refresh`
...
- A done feature offers removal when opened from the board, with branch deletion
  defaulting to yes; nothing is removed without the usual confirmations (ADR 0002).
- The manifest stays at version 1: `pr` is optional and older plugin versions ignore it.
