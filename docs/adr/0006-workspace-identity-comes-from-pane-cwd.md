# Which workspace belongs to a feature is decided by pane working directories
...
A feature is live when some pane's working directory lies inside its root (the plugin's
...
last fallback. Whenever the pane scan resolves a feature, the hint is rewritten.

## Consequences

Renaming a workspace or restarting the server does not lose the association as long as a
shell or agent is still inside the feature folder. A workspace whose every pane has left
